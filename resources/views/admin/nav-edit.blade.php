                            ...
                            <div class="box-body">
                                <div class="form-group {{$errors->has('type')?'has-error':''}}">
                                    ...
                                    <select class="form-control select2" name="type" id="type">
                                        ...
                                        @foreach (\App\Models\Nav::TYPE as $key => $type )
                                            <option value="{{ $key }}"
                                                    @if (!is_null(old('type')) && old( 'type') == $key) selected="selected"
                                                    @elseif( $editNav->type == $key) selected="selected" @endif>{{ $type }}</option>
                                        @endforeach
                                    </select>
                                    @if ($errors->has('type'))
                                    ...
                                    <select class="form-control select2" name="parent_id" id="parent_id">
                                        ...
                                        @foreach($emptyNavs as $empty_nav)
                                            <option value="{{ $empty_nav->id }}"
                                                    @if (!is_null(old('parent_id')) && old( 'parent_id') ==  $empty_nav->id) selected="selected"
                                                    @elseif( $editNav->parent_id == $empty_nav->id) selected="selected" @endif>{{ $empty_nav->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group {{$errors->has('name')?'has-error':''}}">
                                    ...
                                    <input type="text" class="form-control" name="name" id="name" placeholder="请输入菜单名称"
                                           value="{{ old('name')?old('name'):$editNav->name }}">
                                    @if ($errors->has('name'))
                                        <span class="help-block "><strong><i
                                ...
                                <div class="form-group {{$errors->has('url')?'has-error':''}}" id="url-group">
                                    ...
                                    <input type="text" class="form-control" name="url" id="url" placeholder="请输入链接"
                                           value="{{ old('url')?old('url'):$editNav->url }}">
                                ...
                                <div class="form-group {{$errors->has('sort')?'has-error':''}}">
                                    ...
                                    <input type="text" class="form-control" name="sort" id="sort"
                                           placeholder="请输入数字，默认为1"
                                           value="{{ old('sort')?old('sort'):$editNav->sort }}">
                                    @if ($errors->has('sort'))
                                        <span class="help-block "><strong><i
                                    ...
                                    <div class="radio">
                                        <label class="i-checks">
                                            <input type="radio" name="status"
                                                   ...
                                                   @if(!is_null(old('status')) && old( 'status') == \App\Models\Nav::STATUS_DISPLAY) checked="checked"
                                                   @elseif($editNav->status == \App\Models\Nav::STATUS_DISPLAY ) checked="checked" @endif>
                                            &nbsp;显示
                                        </label>
                                        <label class="i-checks">
                                            <input type="radio" name="status" value="{{ \App\Models\Nav::STATUS_HIDE }}"
                                                   @if(!is_null(old('status')) && old( 'status') == \App\Models\Nav::STATUS_HIDE) checked="checked"
                                                   @elseif($editNav->status == \App\Models\Nav::STATUS_HIDE ) checked="checked" @endif>
                                            &nbsp;隐藏
                                        </label>
                                    </div>
